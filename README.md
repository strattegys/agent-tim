...
4. **Live droplet CRM (optional):** **Preferred:** copy **[`.env.docker-dev.example`](.env.docker-dev.example)** to **`COMMAND-CENTRAL/.env`** with **`CC_DOCKER_CRM_DB_HOST`** / **`CC_DOCKER_CRM_DB_PORT`** (Tailscale IP **:5432**) so **`web`** reaches Postgres from inside Docker **without** a host bridge (survives PC restart). **`dev-docker-up.ps1`** prepends **`--env-file .env`** automatically. **Fallback:** **`.\scripts\dev-docker-up.ps1 -UseRemoteCrm`** — bridge/tunnel on **`0.0.0.0:5433`** + **[`docker-compose.dev-remote-crm.yml`](docker-compose.dev-remote-crm.yml)**. Flags **`-UseSshTunnel`** / **`-UseTailscaleBridge`** with **`-UseRemoteCrm`**. Server: **`tools/expose-crm-db-tailscale.sh`**. After bridge-only sleep/reboot: **`cd web && npm run db:reconnect`**.
...
6. Verify: **`cd web && npm run check-crm-db`**. From the dev container (prepend **`--env-file .env`** if you use **`CC_DOCKER_*`**): **`docker compose --env-file .env --env-file web/.env.local -f docker-compose.dev.yml exec web npm run check-crm-db`**.
7. Recreate after env changes: **`docker compose --env-file .env --env-file web/.env.local -f docker-compose.dev.yml up -d --force-recreate`** (omit **`.env`** if unused).
